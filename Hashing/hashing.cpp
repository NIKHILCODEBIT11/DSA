...
// Suppose i have to make an array where atmax element can be 12, and i will have to return count of the element specified by the user
...
int main(){
    ...
    int hash[12]={};       //   Declaring a hash array which can have 13 elements, as atmax element of the upper array is 12
...
//      hash[k]=hash[k]++;      --->     Here, the error is that        hash[k]=hash[k]=hash[k]+1;     due to which it firstly returns the value then updates so it will always remain zero
    ...
    cout<<"The count of "<<3<<" in arr is "<<hash[3];
    return 0;
}
